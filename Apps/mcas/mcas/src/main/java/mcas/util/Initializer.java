...
public class Initializer {
	 ...
	 * @param RESOURCES_PATH  -> The Resources path, should have a transformation
	 *                        and rules folder
	 * @return boolean -> The service was successful
	 */
	public boolean initialize(boolean create_new_data, boolean create_rules, boolean random, VirtGraph graph,
		...
		if (create_new_data) {
			...
			for (String file : this.files) {
				try {
					...
					mapping_template = mapping_template.replace("<*input_json_file*>",
							Paths.get(RESOURCES_PATH, "transformation", file).toString());
					InputStream mappingFile = org.apache.commons.io.IOUtils.toInputStream(mapping_template,
							StandardCharsets.UTF_8);
					String rdfFlux = ToRDF.data2RDF(Paths.get(RESOURCES_PATH, "transformation").toString(), mappingFile,
							"salida.ttl");
					// QueryConf.queryGraphBase + "person#" first If I want to use specific graph
					if (random) {
						rdfFlux = rdfFlux.replace("%3C*act1_id*%3E", UUID.randomUUID().toString());
						rdfFlux = rdfFlux.replace("%3C*act2_id*%3E", UUID.randomUUID().toString());
						rdfFlux = rdfFlux.replace("%3C*des1_id*%3E", UUID.randomUUID().toString());
						rdfFlux = rdfFlux.replace("%3C*des2_id*%3E", UUID.randomUUID().toString());
					}
